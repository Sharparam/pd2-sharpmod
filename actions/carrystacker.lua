...
if not string.find(game_state_machine.current_state_name(game_state_machine), "game") then return end

if not cs.Setup then
    local table = table
    ...
    function PlayerManager:refresh_stack_counter()
        ...
        if count > 0 then
            managers.hud:add_special_equipment({id = "carrystacker", icon = "pd2_loot", amount = count})
        end
    end
